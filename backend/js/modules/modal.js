...
function initModalContainer() {
    if (!modalContainer) {
        ...
        modalContainer.className = 'fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md transition-opacity duration-300';
...
export function showModal(options = {}) {
    ...
    const icons = {
        ...
        error: {
            bg: 'bg-red-100',
            icon: `<svg class="w-12 h-12 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>`
        },
        warning: {
            bg: 'bg-yellow-100',
            icon: `<svg class="w-12 h-12 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    const modalHTML = `
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-300 scale-100" id="customModalContent">
            <div class="p-6">
                ...
                <!-- Buttons -->
                <div class="flex ${showCancel ? 'gap-3' : ''} justify-center">
                    ${showCancel ? `
                        <button
                            id="customModalCancelBtn"
                            class="flex-1 px-5 py-2.5 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300 focus:ring-offset-2 transition-colors"
                        >
                            ${cancelText}
                        </button>
                    ` : ''}
                    <button
                        ...
                        class="${showCancel ? 'flex-1' : 'px-8'} px-5 py-2.5 text-sm font-medium text-white bg-[#224796] rounded-lg hover:bg-[#163473] focus:outline-none focus:ring-2 focus:ring-[#224796] focus:ring-offset-2 transition-colors shadow-sm"
                    >
                        ${confirmText}
                    </button>
                </div>
