            ...
            return `
                <tr class="${isStriped ? 'bg-slate-50' : 'bg-white'} hover:bg-slate-100 transition-colors" data-row-index="${index}" data-gl-code="${row.glCode}" data-row-id="${row.id ?? ''}">
                    ...
                    ${monthKeys.map(month => {
                        ...
                        return `
                            <td class="whitespace-nowrap px-3 py-2 text-xs md:text-sm text-right font-money text-slate-700" data-editable="month" data-month="${month}" data-type="currency" data-value="${value}">
